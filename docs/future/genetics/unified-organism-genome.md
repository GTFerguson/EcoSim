...
**Status:** Phase 1 + Phase 2 complete. Phase 3 architecturally complete; mechanical class collapse remains. Phase 4 not started.
  ...
  - **Phase 3 (class deletion) — remaining:** `Creature` and `Plant` class definitions still exist with their ~157 method implementations. Deletion requires: (a) lifting the 157 methods from `Creature::` to `Organism::` (most delegate to components already — mostly `Creature::` → `Organism::` rename), (b) creating an `OrganismBuilder` factory that returns a pre-configured `Organism` with creature-preset or plant-preset genes and appropriate components attached, (c) updating the ~105 caller files that reference `Creature`/`Plant` by name to use the factory and `Organism` references, (d) deleting the `.hpp/.cpp` files. Mechanical multi-session grind — architecture is complete without it.
