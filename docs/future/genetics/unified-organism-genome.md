...
**Status:** Phase 1 + Phase 2 complete. Phase 3 architecturally complete; mechanical class collapse remains. Phase 4 not started.
  ...
  - **Phase 3 (architectural):** `Organism` base now owns `std::unique_ptr<MobilityComponent|HeterotrophyComponent|AutotrophyComponent|ReproductionComponent|CombatComponent>` and `std::unique_ptr<BehaviorController>`. Creature runtime state (position, direction, hunger/thirst/fatigue/metabolism/mate, combat, gut/burrs) migrated entirely into components — Creature itself retains only subclass-specific identity (archetype, biome adaptation, scent, thermal cache, name/character). Plant wires a `BehaviorController` with `PlantLifecycleTick` at construction so both organism types route through the same behavior pipeline.
  - **Phase 3 (mechanical) — remaining:** `Creature` and `Plant` classes still exist as subclasses, with `Creature` still holding its own `_behaviorController` alongside the Organism-base one. Deleting the classes requires moving Creature-specific utility methods (scent updates, navigation, thermal caching) into reusable components or Organism base methods, then replacing `Creature&` and `Plant&` parameters throughout callers (~30 files: World, rendering, serialization, tests, diagnostics). Future-session grind — the architectural refactor is complete without it.
