...
- [[ecs-vs-oo-for-agent-sims]] — Evidence base for the Phase 3 architecture decision in the unified-organism refactor. Compares Entity-Component-System (archetype and sparse-set flavours) against OO composition for the 1k–100k agent range on modest hardware. Covers shipping sim games (Factorio chunk allocation, Stellaris pop grouping, Cities Skylines 2 activity gating, Dwarf Fortress algorithmic limits, RimWorld ThinkTree), ECS library benchmarks (EnTT, Flecs, Bevy, Specs, Legion), contrarian practitioner perspectives (Moonside Games, Samurai Gunn 2 case study), and the academic frontier (Madrona, TeraAgent, FLAME GPU). Concludes that textbook ECS is overkill below ~100k heterogeneous entities and recommends OO composition with optional archetype pooling deferred until profiling justifies it. Informs [[../future/genetics/unified-organism-genome]].
- [[music-and-audio-direction]] — Evidence base for the sonic aesthetic (instrumentation, composition, dynamic behaviour) that defines how EcoSim feels over long observation sessions. Seed references drawn from *Digimon World 1* (File City Theme, Tropical Jungle Day) — a formative influence for the designer that happens to align cleanly with what an observation-driven sim needs. Informs future composition and soundfont selection for [[../game-concepts/genesis-god-sim|Genesis mode]].
