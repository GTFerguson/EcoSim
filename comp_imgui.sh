#!/bin/bash
# EcoSim Compilation Script with SDL2 + Dear ImGui Support
...
echo "=== EcoSim SDL2 + Dear ImGui Build ==="
...
$CXX -g -std=c++17 \
    ...
    -lncurses $SDL2_LIBS \
    -o EcoSim_ImGui 2>&1 | tee err_imgui

if [ ${PIPESTATUS[0]} -ne 0 ]; then
    echo ""
    echo "❌ Compilation failed. See errors above."
    echo "Error log saved to: err_imgui"
    exit 1
else
    rm -f err_imgui
    echo ""
    echo "✅ Compilation successful!"
    echo "Executable: EcoSim_ImGui"
    echo ""
    echo "Run with: ./EcoSim_ImGui"
    echo ""
    echo "Features:"
    ...
    echo "  - NCurses fallback available"
fi